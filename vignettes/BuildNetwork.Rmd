...
ggplot(dt,aes(x=X,y=Y,label=name))+#geom_point()+
    geom_point(data=dt_vips,
               aes(x=X,y=Y),colour=baseColor,size = 7,shape=15,show.legend=FALSE)+
    geom_point(data=dt_res,
               aes(x=X,y=Y, alpha=(X*Y)), size = 3,shape=16,show.legend=FALSE)+
    geom_label_repel(aes(label=as.vector(lbls)),
                     ...
                     force=1,size=rel(3.8),show.legend=FALSE,max.overlaps=200)+
