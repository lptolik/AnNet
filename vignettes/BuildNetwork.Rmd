...
### Reclustering
There are still too large cluster left. We can recluster them by applying the same, or different clustering algorithm on the cluster subgraph:
```{r recluster}
remem<-calcReclusterMatrix(gg,mem,alg,10)
head(remem)
```

And we can apply second order clustering layout:
```{r plot.recluster.layout}
