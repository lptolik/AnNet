...
gg<-calcClustering(gg,alg = alg)
  ...
  # cl<-rSpectral::spectral(DF=el,fixNeig=1,CnMIN=5)
  # cm       <- data.frame(names=cl$ID,membership=cl$K)
  #   m[,2]<-as.character(cm$membership)
  # gg<-AnNet:::applpMatrixToGraph(gg,m)
...
Build consensus matrix for spectral clustering by perturbing nodes
```{r build.conmat.v}
