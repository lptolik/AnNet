...
lr.diseasome<-readRDS('lr.diseasome.rds')
```
...
## Component size and degree fit to power law
```
{r degree.hdn.powerlaw, fig.width=12,fig.height=12,error=TRUE}
options("show.error.messages"=TRUE)
pFit <- FitDegree( as.vector(igraph::degree(graph=hdn)),plot=TRUE,WIDTH=2480, HEIGHT=2480)
```
...
## Component size fit to power law
```
{r degree.dgn.powerlaw, fig.width=12,fig.height=12,error=TRUE}
pFit <- FitDegree( as.vector(igraph::degree(graph=dgn)),plot=TRUE,WIDTH=2480, HEIGHT=2480)
```
```
...
```{r plot.dgn.pr.bridgenes,fig.width=12,fig.height=12}
...
```{r plot.dgn.sl.bridgenes,fig.width=12,fig.height=12}
VIPs=V(dgnLCC)$name[as.numeric(V(dgnLCC)$DEG)>40]
dgnLCC <- calcCentrality(dgnLCC)
indx   <- match(V(dgnLCC)$name,VIPs)
group <- ifelse( is.na(indx), 0,1)
...
```{r plot.overlap,fig.width=12,fig.height=12,warning=FALSE}
options("show.error.messages"=TRUE)
coverlap<-data.frame(x=c(roverlap,loverlap),
                     type=c('data',rep('random',length(loverlap))))
ggplot(coverlap,aes(x=x,color=type))+geom_density()+
  geom_vline(xintercept = roverlap)+ 
  labs(x = "Number of interactions overlap",
       title='DGN-PPI edge overlap')
```
